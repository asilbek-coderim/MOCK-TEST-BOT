...
if (file_exists(__DIR__ . '/test_module.php')) require_once __DIR__ . '/test_module.php';
...
function menu_kb(): array {
    return ['keyboard' => [
        ...
        [['text' => '🧪 Test ishlash'], ['text' => '📊 Natijam']],
...
function admin_kb(): array {
    return ['inline_keyboard' => [
        ...
        [['text' => '🧪 Testlar', 'callback_data' => 'a_tests']],
...
if (isset($update['message'])) {
    ...
    // Mini App'dan kelgan javoblar
    if (isset($msg['web_app_data']) && function_exists('handle_webapp')) {
        handle_webapp($msg, $uid, $chat);
        exit;
    }

    // Test moduli: admin holatlari
    if ($is_admin && function_exists('test_admin_state')
        && test_admin_state($state, $text, $uid, $chat)) {
        exit;
    }

    // Test moduli: foydalanuvchi tugmalari
    if (function_exists('test_user_button') && test_user_button($text, $uid, $chat)) {
        exit;
    }
...
if (isset($update['callback_query'])) {
    ...
    if (function_exists('test_admin_cb') && test_admin_cb($data, $uid, $chat, $mid)) exit;
